...
AbstractOutput::AbstractOutput(OutputCatalog *catalog)
    : QAbstractTableModel(catalog), _catalog(catalog), _statistics(nullptr),
      _interp(nullptr), _offset_top(0), _offset_bot(0) {
...
auto AbstractOutput::columnCount(const QModelIndex &parent) const -> int {
  ...
  int count =
      needsTime() ? (1 + siteCount()) : (1 + siteCount() * motionCount());
...
auto AbstractOutput::data(const QModelIndex &index, int role) const
    -> QVariant {
  ...
  if (role == Qt::DisplayRole) {
    if (index.column() == 0) {
      ...
      return index.row() < _statistics->average().size()
                 ? QVariant(_statistics->average().at(index.row()))
                 : QVariant();
    } else if (_statistics && _statistics->hasEnoughData() &&
               index.column() == columnCount() - 1) {
      return index.row() < _statistics->stdev().size()
                 ? QVariant(_statistics->stdev().at(index.row()))
                 : QVariant();
    } else {
      return index.row() < _data.at(site).at(motion).size()
                 ? QVariant(_data.at(site).at(motion).at(index.row()))
                 : QVariant("NaN");
    }
  }
...
auto AbstractOutput::headerData(int section, Qt::Orientation orientation,
...
void AbstractOutput::plot(QwtPlot *const qwtPlot,
  ...
  for (int i = 0; i < siteCount(); ++i) {
    for (int j = 0; j < motionCount(); ++j) {
      const QVector<double> &x =
          (curveType() == Yfx) ? ref(j) : _data.at(i).at(j);

      const QVector<double> &y =
          (curveType() == Yfx) ? _data.at(i).at(j) : ref(j);
...
void AbstractOutput::exportData(const QString &path, const QString &separator,
  ...
  for (int m = 0; m < motionCount(); ++m) {
    ...
    for (int i = 0; i < colCount; ++i)
      out << headerData(i, Qt::Horizontal, Qt::DisplayRole).toString()
          << separator;
...
auto AbstractOutput::intToSite(int i) const -> int { return i / motionCount(); }
...
auto AbstractOutput::exportEnabled() const -> bool { return _exportEnabled; }
...
auto AbstractOutput::motionIndex() const -> int { return _motionIndex; }
...
auto AbstractOutput::needsDepth() const -> bool { return false; }

auto AbstractOutput::needsFreq() const -> bool { return false; }

auto AbstractOutput::needsPeriod() const -> bool { return false; }

auto AbstractOutput::needsTime() const -> bool { return false; }

auto AbstractOutput::timeSeriesOnly() const -> bool { return false; }
...
void AbstractOutput::setCurveSamples(QwtPlotCurve *curve,
                                     const QVector<double> &x,
...
auto AbstractOutput::siteIndependent() const -> bool { return false; }

auto AbstractOutput::motionIndependent() const -> bool { return false; }
...
void AbstractOutput::columnToSiteMotion(const int column, int *site,
                                        int *motion) const {
  *site = -1;
  *motion = _motionIndex;
  const int end =
      columnCount() - ((_statistics && _statistics->hasEnoughData()) ? 2 : 0);

  if (0 < column && column < end) {
    // Only find values of the site and motion indices for the data columns
    // (column != 0)
    if (needsTime()) {
      *site = column - 1;
      *motion = _motionIndex;
...
auto AbstractOutput::prefix() const -> const QString { return ""; }

auto AbstractOutput::suffix() const -> const QString { return ""; }
