...
void ResultsPage::setSelectedSeries(const QModelIndex &current,
  ...
  _selectedOutput->setMotionIndex(_selectedOutput->intToMotion(_selectedRow));
  ...
  if (_selectedOutput->needsTime()) {
    _outputTableView->selectColumn(
        _selectedRow / _selectedOutput->motionCount() + 1);
  } else {
    _outputTableView->selectColumn(_selectedRow + 1);
  }
}

void ResultsPage::selectedDataChanged(const QModelIndex &current,
                                      const QModelIndex &previous) {
  Q_UNUSED(previous);
  ...
  if (row >= 0) {
    if (_selectedOutput->needsTime())
      ...
      row =
          row * _selectedOutput->motionCount() + _selectedOutput->motionIndex();
...
void ResultsPage::setSelectedOutput(int index) {
  ...
  // Need to remove the previous connections because otherwise it signals
  // multiple times
  ...
  _enableSitePushButton->setHidden(_selectedOutput->siteIndependent());

  // Motion independent
  _catalogTableView->setColumnHidden(OutputCatalog::MotionColumn,
                                     _selectedOutput->motionIndependent());
  _enableMotionPushButton->setHidden(_selectedOutput->motionIndependent());
...
void ResultsPage::colorCurve(int row) {
  ...
  QPen pen = _outputCatalog->enabledAt(row) ? QPen(QBrush(Qt::darkGreen), 2)
                                            : QPen(QBrush(Qt::darkRed), 2);
...
void ResultsPage::setMotionEnabled(bool enabled) {
  _outputCatalog->setMotionEnabled(_selectedOutput->intToMotion(_selectedRow),
                                   enabled);
...
void ResultsPage::setSiteEnabled(bool enabled) {
  _outputCatalog->setSiteEnabled(_selectedOutput->intToSite(_selectedRow),
                                 enabled);
